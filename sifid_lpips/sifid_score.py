...
"""
#python3 metric/sifid_score.py --path2real metric/data/euv_ch --path2fake AI_generation/sinddm/ch/
import os
import pathlib
...
parser.add_argument('--path2fake', type=str, default='/home/Documents/SinSEM-main/results/ch_SDE_x1_b32_var_1sigma_clip3/final_samples_unbatched_sample_2025-07-29 09_07_48.394271', help=('Path to generated images'))
...
def get_activations(files, model, batch_size=1, dims=64,
    ...
    for i in range(n_batches):
        ...
        # if grayscale convert to RGB, add one more dimension to the end
...
# write a function to convert png to jpg
def png2jpg(path):
    path = pathlib.Path(path)
    files = sorted(list(path.glob('*.png')))
    for i in range(len(files)):
        img = cv2.imread(str(files[i]))  # Convert Path object to string
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # Convert color space from BGR to RGB
        cv2.imwrite(str(files[i].with_suffix('.jpg')), img)  # Save the image as JPEG
...
if __name__ == '__main__':
    ...
    sifid_values = calculate_sifid_given_paths(path1,path2,1,args.gpu!='',2048,suffix)

    sifid_values = np.asarray(sifid_values,dtype=np.float32)
    numpy.save('SIFID', sifid_values)
    print('SIFID cnt:', sifid_values.shape[0])
    # print(sifid_values)
    print('path2:', path2)
